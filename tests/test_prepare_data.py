...
def read_and_diff(ds_reader, path, test_data, measures, dimensions, continuous_obs, basis):
    ...
    for key in dimensions + continuous_obs:
        pd.testing.assert_series_equal(
            test_data.obs[key], prepared_adata.obs[key], check_index=False, check_flags=False
        )
...
def test_prepare_cxg_tile_db(test_data, measures, dimensions, continuous_obs, basis, tmp_path):
    try:
        ...
        subprocess.check_call(
            ["cellxgene", "convert", "-o", output_cxg, "--disable-corpora-schema", output_h5ad]
        )
        read_and_diff(
            TileDBDataset(), output_cxg, test_data, measures, dimensions, continuous_obs, basis
        )
...
def test_prepare(
    test_data, measures, dimensions, continuous_obs, basis, file_format, spatial, tmp_path
):
    ...
    test_data.obs = test_data.obs[dimensions + continuous_obs]
    if spatial:
        test_data.uns["images"] = [dict(type="image", name="tissue_hires", image=output_dir)]
    ...
    if file_format == "parquet":
        ...
        reader = ZarrDataset()
    read_and_diff(reader, output_dir, test_data, measures, dimensions, continuous_obs, basis)

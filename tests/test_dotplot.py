...
def _expected(adata, measure, by):
    ...
    values = np.asarray(
        values.todense() if scipy.sparse.issparse(values) else values
    ).ravel()
    grouped = pd.Series(values, index=adata.obs.index).groupby(
        adata.obs[by].values, observed=True
    )
...
def test_dotplot_accepts_anndata(dotplot_data, measures, by):
    results = DotPlotAggregator(var_measures=measures, dimensions=[by]).execute(
        dotplot_data
    )
...
def test_dotplot_values_match_manual_groupby(dotplot_data, measures, by):
    result = DotPlotAggregator(var_measures=measures, dimensions=[by]).execute(
        dotplot_data
    )[0]
    ...
    for value in result["values"]:
        ...
        np.testing.assert_allclose(
            ...
            expected_mean.loc[categories].values,
            rtol=1e-4,
            atol=1e-5,
        )
        np.testing.assert_allclose(
            np.asarray(value["percentExpressed"]),
            expected_pct.loc[categories].values,
            rtol=1e-4,
        )
...
def test_dotplot_multiple_dimensions(dotplot_data, measures, by):
    adata = dotplot_data
    adata.obs["half"] = pd.Categorical(
        np.where(np.arange(adata.shape[0]) % 2 == 0, "a", "b")
    )
    results = DotPlotAggregator(
        var_measures=measures, dimensions=[[by, "half"]]
    ).execute(adata)
    assert len(results) == 1

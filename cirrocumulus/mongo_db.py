...
class MongoDb(AbstractDB):
    ...
    def category_names(self, email, dataset_id):
        ...
        for doc in collection.find(dict(dataset_id=dataset_id)):
            ...
            cat_results[doc["original"]] = {
                "newValue": doc.get("new"),
    ...
    def user(self, email):
        ...
        if doc is None:
            ...
            return {"id": email, "importer": doc.get("importer", False)}
    ...
    def upsert_dataset(self, email, readers, dataset):
        ...
        if (
            ...
            and not self.capabilities()[SERVER_CAPABILITY_EDIT_DATASET]
        ):
            return
        collection = self.db.datasets
    ...
    def update_job(self, email, job_id, status, result):
        ...
        collection.update_one(
            dict(_id=ObjectId(job_id)), {"$set": dict(status=status, result=result)}
        )
